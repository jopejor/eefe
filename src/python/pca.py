...
name="normalized607Wells86_94RemovedBiologData.csv"
df=pd.read_csv(name)
X = df.iloc[:,4:].values
...
plt.figure()
ig, ax = plt.subplots()
colors = ['royalblue', 'sandybrown', 'firebrick','dimgray']
lw = 2
conditions=unique(y)


for color, i, conditions in zip(colors, ['15', '37', '43','Ancestor'], conditions):
...
pp = PdfPages('pca_607.pdf')
plt.savefig(pp,format='pdf')
pp.close()

np.savetxt("foo607.csv", X_r, delimiter=",")
